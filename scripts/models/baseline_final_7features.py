"""
最終版ベースラインモデル（7特徴量）
冗長な特徴量をすべて削除：Title, IsAlone, TicketGroupSize, FarePerPerson
"""
...
def main():
    ...
    print("  - Title_* → SexとAgeから派生")
    print("  - IsAlone → FamilySizeから派生")
    print("  - TicketGroupSize → データリーク")
    print("  - FarePerPerson → Fareと相関0.84、冗長")
    print("\n最終特徴量:")
    ...
    submission_path = 'submissions/baseline_final_7features_ensemble.csv'
    ...
    for model_name, test_pred in test_predictions_dict.items():
        ...
        model_path = f'submissions/baseline_final_7features_{model_name.lower()}.csv'
    ...
    print("=" * 80)
    print(f"\n特徴量削減の経緯:")
    print(f"  20特徴 → 14特徴 → 10特徴 → 9特徴 → 8特徴 → 7特徴")
    print(f"\n最終版 (7特徴): OOF {oof_ensemble_acc:.4f}, LB ???")
    print(f"\n冗長性とデータリークを完全に排除したクリーンなモデル")
