...
def create_features_with_improved_age(df, train_df=None):
    """改善されたAge補完を用いた特徴量作成"""
    data = df.copy()
...
def main():
    ...
    # 特徴量作成
    print("\n[2] 特徴量作成（Pclass x Title補完）")
    train_features = create_features_with_improved_age(train)
    test_features = create_features_with_improved_age(test, train_df=train)

    feature_cols = get_feature_columns()
    print(f"使用する特徴量: {len(feature_cols)}個")
    print("\nAge補完方法:")
    print("  1. Pclass x Title別の中央値")
    print("  2. フォールバック: Title別の中央値")
    print("  3. 最終フォールバック: 全体の中央値")
    print("\n例: Pclass 1のMr → 41.5歳、Pclass 3のMr → 26.0歳")
    ...
    submission_path = 'submissions/baseline_improved_age_ensemble.csv'
    ...
    for model_name, test_pred in test_predictions_dict.items():
        ...
        model_path = f'submissions/baseline_improved_age_{model_name.lower()}.csv'
    ...
    print("=" * 80)
    print(f"\n比較:")
    print(f"  Title補完のみ:        OOF 0.8507, LB 0.75119")
    print(f"  Pclass+Title補完:     OOF {oof_ensemble_acc:.4f}, LB ???")
    print(f"\nより精密な年齢推定により、LBスコアの改善を期待")
