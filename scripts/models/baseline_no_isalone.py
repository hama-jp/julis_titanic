"""
IsAloneを削除したベースラインモデル（9特徴量）
IsAloneはFamilySizeから派生した冗長な情報
"""
...
def create_features_no_isalone(df, train_df=None):
    """IsAloneなしの特徴量作成"""
    data = df.copy()
...
def main():
    print("=" * 80)
    print("IsAloneを削除したベースラインモデル（9特徴量）")
    print("=" * 80)
    ...
    # 特徴量作成
    print("\n[2] 特徴量作成（IsAloneを削除）")
    train_features = create_features_no_isalone(train)
    test_features = create_features_no_isalone(test, train_df=train)

    feature_cols = get_feature_columns()
    print(f"使用する特徴量: {len(feature_cols)}個（10個→9個に削減）")
    print("\n削除した特徴量:")
    print("  - IsAlone")
    print("  理由: IsAloneはFamilySize==1から派生した冗長な情報")
    print("\n残した特徴量:")
    ...
    submission_path = 'submissions/baseline_no_isalone_ensemble.csv'
    ...
    for model_name, test_pred in test_predictions_dict.items():
        ...
        model_path = f'submissions/baseline_no_isalone_{model_name.lower()}.csv'
    ...
    print("=" * 80)
    print(f"\n比較:")
    print(f"  IsAloneあり (10特徴): OOF 0.8373, LB 0.75119")
    print(f"  IsAloneなし (9特徴):  OOF {oof_ensemble_acc:.4f}, LB ???")
    print(f"\n特徴量を1つ削減し、過学習をさらに軽減")
