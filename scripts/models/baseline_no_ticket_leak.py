"""
TicketGroupSizeを削除したモデル（データリーク対策）
TicketGroupSizeはtrain+testで計算しているためデータリークの可能性
"""
...
def create_features_no_ticket_leak(df, train_df=None):
    """TicketGroupSizeなしの特徴量作成（データリーク対策）"""
    data = df.copy()
    ...
    # TicketGroupSizeは削除（データリーク対策）

    return data


def get_feature_columns():
    """使用する特徴量（TicketGroupSizeなし）"""
    features = [
        'Pclass',
        'Sex_male',
        'Age_fillna',
...
def main():
    print("=" * 80)
    print("TicketGroupSizeを削除したモデル（データリーク対策）")
    print("=" * 80)
    ...
    # 特徴量作成
    print("\n[2] 特徴量作成（TicketGroupSizeを削除）")
    train_features = create_features_no_ticket_leak(train)
    test_features = create_features_no_ticket_leak(test, train_df=train)

    feature_cols = get_feature_columns()
    print(f"使用する特徴量: {len(feature_cols)}個（9個→8個に削減）")
    print("\n削除した特徴量:")
    print("  - TicketGroupSize")
    print("  理由: train+testを結合して計算しているためデータリークの可能性")
    print("\n残した特徴量:")
    ...
    submission_path = 'submissions/baseline_no_ticket_leak_ensemble.csv'
    ...
    for model_name, test_pred in test_predictions_dict.items():
        ...
        model_path = f'submissions/baseline_no_ticket_leak_{model_name.lower()}.csv'
    ...
    print("=" * 80)
    print(f"\n比較:")
    print(f"  TicketGroupSizeあり (9特徴): OOF 0.8384, LB 0.75598")
    print(f"  TicketGroupSizeなし (8特徴): OOF {oof_ensemble_acc:.4f}, LB ???")
    print(f"\nデータリークを排除し、LBスコアの大幅な改善を期待")
