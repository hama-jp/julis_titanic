"""
最適化ベースラインモデル - 有効な特徴量のみを厳選してLB 0.8を目指す
シンプル版(OOF 0.8451)に戻り、効果的な特徴量のみを追加
"""
...
def create_optimized_features(df, train_df=None):
    ...
    # === 追加特徴量（効果的なもののみ）===
    # 9. FarePerPerson: 一人当たりの運賃
    data['FarePerPerson'] = data['Fare_fillna'] / data['FamilySize']

    # 10. TicketGroupSize: 同じチケットを持つ人数
    if train_df is not None:
    ...
    # 11. Age_Pclass: 年齢とクラスの交互作用
    data['Age_Pclass'] = data['Age_fillna'] * data['Pclass']

    # 12. Fare_Pclass: 運賃とクラスの交互作用
...
def main():
    print("=" * 80)
    print("最適化ベースラインモデル - 有効な特徴量のみを厳選")
    print("=" * 80)
    ...
    print("基本特徴 (16個): Pclass, Sex, Age, SibSp, Parch, FamilySize, IsAlone,")
    print("                 Fare, Embarked, Title")
    print("追加特徴 (4個):  FarePerPerson, TicketGroupSize, Age_Pclass, Fare_Pclass")
    ...
    submission_path = 'submissions/baseline_optimized_ensemble.csv'
    ...
    submission_top3_path = 'submissions/baseline_optimized_top3_weighted.csv'
    ...
    for model_name, test_pred in test_predictions_dict.items():
        ...
        model_path = f'submissions/baseline_optimized_{model_name.lower()}.csv'
    ...
    print("=" * 80)
    print(f"\nベースライン比較:")
    print(f"  シンプル版(16特徴): OOF 0.8451, AUC 0.8810")
    print(f"  改善版(33特徴):     OOF 0.8395, AUC 0.8829")
    print(f"  最適化版(20特徴):   OOF {oof_ensemble_acc:.4f}, AUC {oof_ensemble_auc:.4f}")
